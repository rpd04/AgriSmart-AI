...
public class MarketplaceController : ControllerBase
{
    ...
    public async Task<ActionResult<List<ListingResponse>>> GetActiveListings()
    {
        var listings = await _db.MarketplaceListings
            ...
            .Select(l => new ListingResponse(l.Id, l.CropType, l.QuantityKg, l.SuggestedPrice, l.Status, l.CreatedAt, l.User!.Name))
            .ToListAsync();
        return Ok(listings);
    }
    ...
    public async Task<ActionResult<ListingResponse>> CreateListing(CreateListingRequest request)
    {
        ...
        return Ok(new ListingResponse(listing.Id, listing.CropType, listing.QuantityKg, listing.SuggestedPrice, listing.Status, listing.CreatedAt, user?.Name ?? ""));
    }
}
